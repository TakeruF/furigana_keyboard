...
地球儀キーとIME切替は端末上でしか確認できないため、リリースごとに次を実行します。

1. 地球儀キーのタップで次のキーボードへ移り、もう一度タップで戻れること。長押しでキーボード選択画面が開くこと。
2. 他キーボードとの往復（Gboard → ふりがな → Gboard → ふりがな）で、未確定文字、文節、ローマ字カーソル、候補が残らないこと。
3. アプリAからアプリBへ移動しても同様に状態が残らないこと。
4. かな → 英語 → 手書きとパネルを切り替え、最後に使った日本語パネルが次回復元されること。
5. パスワード欄で候補が出ず、未確定文字を持たずに1文字ずつ確定すること。欄を離れた後の通常欄で変換が元通り動くこと。
6. Androidの設定 → キーボード → ふりがなキーボード → 設定から、アプリの設定画面が開くこと。
7. キーボード選択UIとIME一覧で、サブタイプ名が意図した表記で出ること。
8. 横向きでも入力欄がフルスクリーンの抽出UIに置き換わらず、アプリ本体が見えたままであること。
...
地球儀キー、パネル切替、パスワード欄の扱いは実機でしか確認できないため、リリースごとに次を実行します。

1. 地球儀キーのタップで次のキーボードへ移り、長押しでキーボード一覧が開くこと。
2. 他キーボードとの往復（標準 → ふりがな → 標準 → ふりがな）で、未確定文字、文節、候補が残らないこと。
3. アプリAからアプリBへ移動しても同様に状態が残らないこと。
4. 手書き → かな → 英字 → 記号とパネルを切り替え、設定した初期パネルが次回復元されること。
5. パスワード欄で候補と変換が出ず、未確定文字を持たずに1文字ずつ確定すること。手書きは候補が出て入力できること。欄を離れた後の通常欄で変換が元通り動くこと。
6. ローマ字入力中にスペースキーを長押ししてドラッグすると、カーソルが1文字ずつ動くこと。カーソルを途中へ動かして変換したとき、右側の文字が消えたり巻き込まれたりせず、次の未確定文字列として残ること。
7. サロゲートペアの漢字、濁点付きの合成文字、絵文字の連結列が、削除でもカーソル移動でも1文字として扱われること。
8. フルアクセスが無効のまま、App Groupの更新済み辞書を読めること。
9. iPhoneとiPadの縦横どちらでもレイアウトが崩れないこと。
