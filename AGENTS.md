...
| OSSオブジェクトURL | `https://downloads.hanlu.app/1.0.0-rc.1.apk` | APK名と一致する不変URL |

このプロジェクトでは配布元が専用ドメインのため、APK名は`<versionName>.apk`に統一します。将来同じパスで複数製品やプラットフォームの成果物を扱う場合は、製品別ディレクトリまたは製品名プレフィックスへの移行を検討します。既存の公開契約やユーザー指定と異なる命名へ変更するときは、互換性と移行方法を先に確認してください。
